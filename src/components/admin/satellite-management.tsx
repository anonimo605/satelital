...
const SatelliteManagement = () => {
    ...
    const onAddSubmit = async (values: z.infer<typeof satelliteSchema>) => {
        try {
            ...
            toast({ title: "Satélite Creado", description: `El satélite "${values.name}" ha sido añadido.` });
            addForm.reset();
            setSatelliteImageDataUrl(null);
            ...
            toast({ variant: "destructive", title: "Error", description: "No se pudo crear el satélite." });
        }
    };
    ...
    const onEditSubmit = async (values: z.infer<typeof satelliteSchema>) => {
        ...
        try {
            ...
            toast({ title: "Satélite Actualizado" });
            setEditingSatellite(null);
        } catch (error) {
            console.error("Error updating satellite: ", error);
            toast({ variant: "destructive", title: "Error", description: "No se pudo actualizar el satélite." });
        }
    };
    ...
    const handleDelete = async (satelliteId: string) => {
        try {
            ...
            toast({ title: "Satélite Eliminado" });
        } catch (error) {
            console.error("Error deleting satellite: ", error);
            toast({ variant: "destructive", title: "Error", description: "No se pudo eliminar el satélite." });
        }
    };
    ...
    return (
        <div className="space-y-6">
            <Card>
                <CardHeader>
                    <CardTitle>Añadir Nuevo Satélite</CardTitle>
                </CardHeader>
                <CardContent>
                    <Form {...addForm}>
                        <form onSubmit={addForm.handleSubmit(onAddSubmit)} className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-start">
                                <FormField control={addForm.control} name="name" render={({ field }) => (
                                    <FormItem><FormLabel>Nombre del Satélite</FormLabel><FormControl><Input placeholder="Sputnik 1" {...field} /></FormControl><FormMessage /></FormItem>
                                )} />
                             ...
                             <FormField
                                ...
                                render={({ field }) => (
                                    ...
                                    <div className="space-y-0.5">
                                        ...
                                        <CardDescription>Activa un contador para este satélite.</CardDescription>
                                    </div>
                                    <FormControl>
                                        <Switch
                                        checked={field.value}
                            ...
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    <Label htmlFor="satellite-image-upload">Imagen del Satélite</Label>
                                    <Input id="satellite-image-upload" type="file" accept="image/*" onChange={(e) => handleFileChange(e, 'add')} />
                                    <p className="text-xs text-muted-foreground">Sube una imagen para el satélite. Si no se selecciona una, se usará una por defecto.</p>
                                </div>
            ...
            <Card>
                <CardHeader>
                    <CardTitle>Satélites Existentes</CardTitle>
                    <CardDescription>Lista de todos los satélites disponibles en la plataforma.</CardDescription>
                </CardHeader>
                <CardContent>
                                                ...
                                                <AlertDialog>
                                                    ...
                                                    <AlertDialogContent>
                                                        <AlertDialogHeader>
                                                            ...
                                                            <AlertDialogDescription>
                                                                Esta acción no se puede deshacer. Esto eliminará permanentemente el satélite.
                                                            </AlertDialogDescription>
                                                        </AlertDialogHeader>
                                                        <AlertDialogFooter>
            ...
            <Dialog open={!!editingSatellite} onOpenChange={(isOpen) => !isOpen && setEditingSatellite(null)}>
                <DialogContent className="sm:max-w-3xl">
                     <DialogHeader>
                        <DialogTitle>Editar Satélite</DialogTitle>
                        <DialogDescription>
                           Realiza cambios en el satélite. Haz clic en guardar cuando termines.
                        </DialogDescription>
                    </DialogHeader>
                    <Form {...editForm}>
                        <form onSubmit={editForm.handleSubmit(onEditSubmit)} className="space-y-4 py-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <FormField control={editForm.control} name="name" render={({ field }) => (
                                    <FormItem><FormLabel>Nombre del Satélite</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                                )} />
                            ...
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    <Label htmlFor="edit-satellite-image-upload">Imagen del Satélite</Label>
